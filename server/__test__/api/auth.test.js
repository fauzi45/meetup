...
describe("Auth", () => {
  ...
  describe("Register", () => {
    ...
    test("Should Return 400: Bad Request Data", async () => {
      ...
      await Request(server).post(apiUrl).send(body).expect(400);
    });
  ...
  describe("Login", () => {
    beforeEach(() => {
      ...
      payload = {
        email: "U2FsdGVkX1/RYwxRCGyJnonD7hLS4IVfWS0qgiuzvpw=",
        password: "U2FsdGVkX18re0uwccbpqjStkVLAnV9RXWOlJ5tgyRw=",
      };
      mockUser = _.cloneDeep(MockUser);
      getUser = jest.spyOn(db.User, "findOne");
    });
    ...
    test("Should Return 200: Login Success", async () => {
      ...
      await Request(server)
        ...
        .expect(200)
        .then((res) => {
          expect(!_.isEmpty(res)).toBeTruthy();
        });
    });
